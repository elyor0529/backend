...
func (c *Client) writePump() {
	...
	defer func() {
		ticker.Stop()
		if err := c.conn.Close(); err != nil {
			log.Println("unable to close the connection", err)
		}
	}()
	for {
		select {
		case message, ok := <-c.send:
			if err := c.conn.SetWriteDeadline(time.Now().Add(writeWait)); err != nil {
				log.Println("unable to set connection write deadline", err)
				return
			}
			if !ok {
				// The hub closed the channel.
				if err := c.conn.WriteMessage(websocket.CloseMessage, []byte{}); err != nil {
					log.Println("unable to write close message", err)
				}
				return
			}
		...
		case <-ticker.C:
			if err := c.conn.SetWriteDeadline(time.Now().Add(writeWait)); err != nil {
				log.Println("unable to set write deadline", err)
				return
			}
			if err := c.conn.WriteMessage(websocket.PingMessage, []byte{}); err != nil {
				log.Println("unable to write to connection", err)
				return
			}
		}
	}
